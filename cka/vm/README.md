...
**Start the VM in the foreground** (console output streams to your terminal, useful for watching cloud-init progress on first boot):

```bash
~/cka-lab/node1/start-node1.sh
```

To detach from the console without killing the VM, press `Ctrl+A` then `X` to exit the QEMU monitor.

**Start the VM in the background** (headless, log output to a file):

```bash
nohup ~/cka-lab/node1/start-node1.sh &> ~/cka-lab/node1/node1.log &
```
